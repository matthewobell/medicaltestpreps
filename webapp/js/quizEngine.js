...
// Shuffle array order (used for questions and answer options)
...
// Read the module name from the URL filename, e.g. "module5.json" → "Module 5"
function getModuleTitle(){
  ...
  if(!base) return "Quiz";
  // Convert snake_case / camelCase / dashes to readable label
  // e.g. "module5" → "Module 5", "fls_module2" → "Fls Module 2"
  return base
    .replace(/[-_]/g, " ")
    .replace(/([a-z])(\d)/g, "$1 $2")
...
function showQuestion(){
  ...
  // --- Change 1: Shuffle answer options so correct answer isn't always same position ---
...
// --- Change 2: SVG checkmark / X icon helpers (matching review page style) ---
function correctIconSVG(){
  return `<div class="feedback-icon-circle correct">
...
function incorrectIconSVG(){
  return `<div class="feedback-icon-circle incorrect">
    <svg width="22" height="22" viewBox="0 0 20 20" fill="none">
...
// --- Change 3: Consistent SVG chevrons ---
function chevronDown(){
  return `<svg class="chevron-svg" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
function chevronUp(){
  return `<svg class="chevron-svg" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M18 15l-6-6-6 6" stroke="#333" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
  </svg>`;
}
  ...
  feedbackCard.innerHTML = `
    ...
    ${!isCorrect ? `
      ...
      <div class="answer-body">
        ${correctOptions.map(o => cleanAnswerText(o.text || o)).join("<br>")}
      </div>
    ` : ``}
...
function showReviewPage(){
  ...
  // --- Change 4: Update title to module name on review page too ---
  const titleEl = document.getElementById("page-title");
  if(titleEl) titleEl.innerText = getModuleTitle();
